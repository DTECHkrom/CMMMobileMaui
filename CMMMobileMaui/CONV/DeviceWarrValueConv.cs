...
{
    public class DeviceWarrValueConv : IValueConverter, IMarkupExtension
    {
        public object Convert(object value, Type targetType, object parameter, CultureInfo culture)
        {
            if(value != null)
            {
                ...
                if(warr.WarrantyID == 1)
                {
                    ...
                    {
                        if(long.TryParse(warr.Param1, out long val))
                        {
                            toEnd = (val - warr.Current_Cycles.Value);
                        }
                    }
                }
        ...
        private static DeviceWarrValueConv _conv = null;

        public object ProvideValue(IServiceProvider serviceProvider)
        {
            if(_conv == null)
            {
                _conv = new DeviceWarrValueConv();
            }
