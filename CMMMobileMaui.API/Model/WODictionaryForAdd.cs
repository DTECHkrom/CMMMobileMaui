...
{
    public class WODictionaryForAdd : WODictionaryBase
        , IWODictionary
    {
        ...
        public WODictionaryForAdd(List<WODictResponse> wOCategoryList
        ...
        {
            WOStateList = new List<DictBase>();

            if (HasListDefaultValue(WOReasonMainList))
            {
                WOReasonMainList = new List<WODictResponse>();
            }

            if (HasListDefaultValue(WOCategoryMainList))
            {
                WOCategoryMainList = new List<WODictResponse>();
            }

            WOReasonList = GetForDeviceCategory(WOReasonMainList, device).ToDictBaseList();
            WOCategoryList = GetForDeviceCategory(WOCategoryMainList, device).ToDictBaseList();
